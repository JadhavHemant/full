...
const verify2FALogin = async (req, res) => {
  try {
    ...
    if (verified) {
      ...
      const userResult = await appPool.query(`
        SELECT u."UserId", u."Name", u."Email", u."RoleId",
               u."CompanyId", u."UserTypeId", u."Status", u."IsActive",
               u."EmailVerified", r."RoleName"
        FROM "Users" u
      ...
      await logLoginAttempt({
        userId: user.UserId, email: user.Email, loginStatus: 'success',
        ipAddress: req.ip || req.connection?.remoteAddress || '0.0.0.0',
        userAgent: req.headers['user-agent'] || 'Unknown',
        deviceId: crypto.createHash('sha256').update(`${req.headers['user-agent'] || 'Unknown'}${req.ip || '0.0.0.0'}`).digest('hex').substring(0, 32),
        browser: 'Unknown', deviceType: 'desktop',
        sessionId,
      });
