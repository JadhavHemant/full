// src/Components/AdminSite/HomePage/HomePage.jsx
// Prometheus + Grafana API Tracking Dashboard — Full Monitoring View
...
import { buildCrmNotificationLink } from "../../../features/crm/utils/notificationLinks";
...
const HomePage = () => {
  ...
  const isUserPortal = useMemo(
    () => window.location.pathname.toLowerCase().startsWith("/user"),
    []
  );

  // ── Demo / mock data (replace with real Prometheus queries) ──
  const MOCK = {
    requests: {
      total: "1,243,867",
      rps: 342,
      errorRate: 2.1,
      p99: 438,
      trend: [{ t: "13:00", v: 280 }, { t: "13:15", v: 310 }, { t: "13:30", v: 295 }, { t: "13:45", v: 340 }, { t: "14:00", v: 325 }, { t: "14:15", v: 360 }, { t: "14:30", v: 342 }],
      errorTrend: [{ t: "13:00", e: 0.8 }, { t: "13:15", e: 0.6 }, { t: "13:30", e: 1.2 }, { t: "13:45", e: 2.8 }, { t: "14:00", e: 3.6 }, { t: "14:15", e: 2.2 }, { t: "14:30", e: 2.1 }],
      latencyTrend: [{ t: "13:00", p50: 72, p90: 180, p99: 320 }, { t: "13:15", p50: 78, p90: 195, p99: 350 }, { t: "13:30", p50: 82, p90: 205, p99: 380 }, { t: "13:45", p50: 88, p90: 220, p99: 420 }, { t: "14:00", p50: 84, p90: 214, p99: 438 }, { t: "14:15", p50: 80, p90: 210, p99: 410 }, { t: "14:30", p50: 84, p90: 214, p99: 438 }],
      statusBreakdown: [
    ...
    inventory: {
      ...
      warehouses: [
        { name: "Main WH", pct: 42 }, { name: "East WH", pct: 28 }, { name: "West WH", pct: 18 }, { name: "Others", pct: 12 },
      ],
  ...
  const fetchCompanies = async () => {
    try {
      ...
      setCompanies(rows);
      if (rows.length && !selectedCompanyId) setSelectedCompanyId(String(rows[0].Id));
    } catch (error) {
  ...
  // ── Layout ──
  return (
    <div style={{ minHeight: "100vh", background: colors.bg, color: colors.text, padding: 16, fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif" }}>
      <div style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 1400, margin: "0 auto" }}>
        ...
        <div className="content-card p-5" style={{ background: colors.card }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
            <div>
              <h2 style={{ fontSize: 22, fontWeight: 700, color: colors.text }}>📊 API Monitoring Dashboard</h2>
              <p style={{ fontSize: 13, color: colors.textMuted, marginTop: 4 }}>
                Prometheus-powered metrics — real-time and historical API, Inventory, CRM & System tracking
              </p>
            </div>
            <div style={{ maxWidth: 280, minWidth: 200 }}>
              <label style={{ display: "block", fontSize: 12, fontWeight: 600, color: colors.textMuted, marginBottom: 4 }}>Company</label>
              <select
                value={selectedCompanyId}
                onChange={(e) => setSelectedCompanyId(e.target.value)}
                style={{ width: "100%", padding: "8px 12px", borderRadius: 8, border: `1px solid ${colors.border}`, background: colors.card, color: colors.text, fontSize: 13, outline: "none" }}
              >
                {companies.map((c) => (
        ...
        {/* ══════════════════════════════════════════════ */}
        <SectionLabel>🚀 API Overview — last 1 hour</SectionLabel>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
          <StatCard label="Total requests" big={MOCK.requests.total} delta="+8.3% vs prev hour">
            <ResponsiveContainer width="100%" height={36}>
              <AreaChart data={MOCK.requests.trend}>
                <defs>
                  <linearGradient id="g1" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={C.success} stopOpacity={0.25}/><stop offset="100%" stopColor={C.success} stopOpacity={0.02}/></linearGradient>
                </defs>
                <Area type="monotone" dataKey="v" stroke={C.success} strokeWidth={2} fill="url(#g1)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </StatCard>
          <StatCard label="Requests / sec" big={MOCK.requests.rps} delta="+12 rps since 5m ago">
            <ResponsiveContainer width="100%" height={36}>
              <AreaChart data={MOCK.requests.trend}>
                <defs><linearGradient id="g2" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={C.success} stopOpacity={0.25}/><stop offset="100%" stopColor={C.success} stopOpacity={0.02}/></linearGradient></defs>
                <Area type="monotone" dataKey="v" stroke={C.success} strokeWidth={2} fill="url(#g2)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </StatCard>
          <StatCard label="Error rate" big={<span style={{color: C.danger}}>{MOCK.requests.errorRate}%</span>} delta="+0.4% spike at 14:32" deltaType="down">
            <ResponsiveContainer width="100%" height={36}>
              <AreaChart data={MOCK.requests.errorTrend}>
                <defs><linearGradient id="g3" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={C.danger} stopOpacity={0.25}/><stop offset="100%" stopColor={C.danger} stopOpacity={0.02}/></linearGradient></defs>
                <Area type="monotone" dataKey="e" stroke={C.danger} strokeWidth={2} fill="url(#g3)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </StatCard>
        ...
        {/* HTTP Status + Latency Percentiles */}
        ...
        {/* Stock movements, P&L, Units/Taxes, Audit */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <div className="content-card p-4" style={{ background: colors.card }}>
            ...
            <MiniTable headers={["Module", "Create", "Update", "Delete", "Total"]} rows={[...SYS.crud, { module: "Total", create: SYS.crud.reduce((a, b) => a + b.create, 0), update: SYS.crud.reduce((a, b) => a + b.update, 0), delete: SYS.crud.reduce((a, b) => a + b.delete, 0) }]} renderRow={(r, i, isTotal) => (
              <>
          ...
          <StatCard label="Upstream timeout rate" big={<span style={{color: C.warning}}>0.7%</span>} delta="+0.2% in last 10m" deltaType="down">
            <ResponsiveContainer width="100%" height={36}>
              <AreaChart data={[{t:1,v:0.3},{t:2,v:0.4},{t:3,v:0.5},{t:4,v:0.4},{t:5,v:0.6},{t:6,v:0.7},{t:7,v:0.7}]}>
                <defs><linearGradient id="g5" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={C.warning} stopOpacity={0.25}/><stop offset="100%" stopColor={C.warning} stopOpacity={0.02}/></linearGradient></defs>
                <Area type="monotone" dataKey="v" stroke={C.warning} strokeWidth={2} fill="url(#g5)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </StatCard>
          ...
          <StatCard label="Rate limit hits / min" big={<span style={{color: C.danger}}>43</span>} delta="429 responses rising" deltaType="down">
            <ResponsiveContainer width="100%" height={36}>
              <AreaChart data={[{t:1,v:10},{t:2,v:12},{t:3,v:15},{t:4,v:22},{t:5,v:30},{t:6,v:38},{t:7,v:43}]}>
                <defs><linearGradient id="g6" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={C.danger} stopOpacity={0.25}/><stop offset="100%" stopColor={C.danger} stopOpacity={0.02}/></linearGradient></defs>
                <Area type="monotone" dataKey="v" stroke={C.danger} strokeWidth={2} fill="url(#g6)" dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </StatCard>
        </div>
        ...
        <div style={{ textAlign: "center", padding: "16px 0", fontSize: 11, color: colors.textMuted }}>
          Data sourced from Prometheus • Metrics refresh every 15s • Dashboard auto-updates
        </div>
