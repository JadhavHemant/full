...
const HSNCodesPage = () => {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const response = await axiosInstance.get("/hsn-codes");
      setData(response.data?.data || response.data || []);
    } catch (error) {
      toast.error("Failed to fetch HSN/SAC Codes");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedItem) {
        ...
        toast.success("HSN/SAC Codes updated successfully");
      } else {
        await axiosInstance.post("/hsn-codes", formData);
        toast.success("HSN/SAC Codes created successfully");
      }
      setShowModal(false);
      fetchData();
    } catch (error) {
      toast.error("Failed to save HSN/SAC Codes");
    }
  };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="flex justify-between items-center p-6 border-b">
              <h2 className="text-2xl font-bold text-gray-900">{selectedItem ? 'Edit' : 'Add'} HSN/SAC Codes</h2>
              <button
