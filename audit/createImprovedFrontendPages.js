/**
 * Script to create improved frontend pages with better UI/UX
 */
...
// Improved template with better UI/UX
const createImprovedPageTemplate = (pageName, title, apiEndpoint) => {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="flex justify-between items-center p-6 border-b">
              <h2 className="text-2xl font-bold text-gray-900">{selectedItem ? 'Edit' : 'Add'} ${title}</h2>
              <button
...
// Overwrite existing pages with improved versions
pages.forEach(page => {
  const filePath = path.join(pagesDir, page.name + '.jsx');
  if (fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, createImprovedPageTemplate(page.name, page.title, page.api));
    console.log('✅ Updated ' + page.name + '.jsx');
  } else {
    fs.writeFileSync(filePath, createImprovedPageTemplate(page.name, page.title, page.api));
...
reportPages.forEach(page => {
  ...
  if (fs.existsSync(filePath)) {
    fs.writeFileSync(filePath, createImprovedPageTemplate(page.name, page.title, page.api));
    console.log('✅ Updated reports/' + page.name + '.jsx');
  } else {
    fs.writeFileSync(filePath, createImprovedPageTemplate(page.name, page.title, page.api));
...
console.log('\n🎉 Updated ' + (pages.length + reportPages.length) + ' pages with improved UI!');